...
if (($chosen==1));then
        ...
        yum -y install ntpdate && timedatectl set-timezone Asia/Shanghai && ntpdate ntp1.aliyun.com && systemctl start supervisord;systemctl disable firewalld && systemctl stop firewalld && yum -y update && ./uykb.sh
elif (($chosen==9));then
