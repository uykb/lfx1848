...
if (($chosen==1));then
        ...
        (yum install ca-certificates wget -y && update-ca-trust force-enable) || (apt-get install ca-certificates wget -y && update-ca-certificates)
        wget -N --no-check-certificate "https://raw.githubusercontent.com/uykb/lfx1848/main/tcp.sh" && chmod +x tcp.sh && ./tcp.sh
elif (($chosen==3));then
